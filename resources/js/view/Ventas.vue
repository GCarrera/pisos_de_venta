	...
	methods:{
		   ...
		   //OBTENEMOS LOS PRODCUTOS QUE HALLAN SIDO ANULADOS
			axios.get('/api/get-ventas-anuladas').then(response => {
				...
				if (ventas.length > 0) {
					...
					axios.post('http://mipuchito/api/actualizar-anulados', {ventas: ventas, piso_venta: this.id}).then(response => {//WEB
		...
		//VENTAS
				...
				axios.get('/api/get-piso-venta-id').then(response => {
					...
					axios.get('http://mipuchito/api/ultima-venta/'+piso_venta_id).then(response => {//WEB
						...
						axios.get('/api/ventas-sin-registrar/'+piso_venta_id+'/'+ultima_venta).then(response => {
							...
							axios.post('http://mipuchito/api/registrar-ventas', {ventas: ventas, piso_venta_id: piso_venta_id}).then(response => {
				...
				axios.put('http://mipuchito/api/actualizar-dinero-piso-venta/'+this.id, {dinero: this.piso_venta_selected.dinero}).then(response => {//EN LA WEB
				...
				axios.get('http://mipuchito/api/ultima-vaciada-caja/'+this.id).then(response => {//WEB
					...
					axios.get('/api/ultima-vaciada-caja-local/'+ultima_caja.id_extra).then(response => {
						...
						if (cajas.length > 0) {
							...
							axios.post('http://mipuchito/api/registrar-cajas', {cajas: cajas}).then(response => {//WEB
		...
		sync_anular(){
			...
			axios.get('/api/get-piso-venta-id').then(response => {
				...
				axios.get('/api/get-ventas-anuladas').then(response => {
					...
					if (ventas.length > 0) {
					//ACTUALIZAMOS LOS ANULADOS EN LA WEB
						axios.post('http://mipuchito/api/actualizar-anulados', {ventas: ventas, piso_venta: piso_venta_id}).then(response => {//WEB
