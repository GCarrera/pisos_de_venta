	...
	export default{
		...
		methods:{
			resumen(){
				...
				axios.get(location.origin + '/api/resumen').then(response => {
			...
			vaciar_caja(){
				...
				axios.post(location.origin + '/api/vaciar-caja').then(response => {

					console.log(response.data);

					this.piso_venta_selected = response.data.piso_venta;
					window.location="http://localhost/pisos_de_venta/public/home";
				}).catch(e => {
			...
			sincronizar(){
				...
				axios.post(location.origin + '/api/sincronizacion', {id: this.id}).then(response => {
					...
					axios.post('http://mipuchito.com/api/sincronizacion', {id: this.id}).then(response => {//WEB
						...
						//window.location="http://localhost/pisos_de_venta/publichttp://127.0.0.1:8000/home";
					}).catch(e => {
						console.log(e.response);
						this.error = true;
						this.cambiar()
